<?php 
	$english = array(
		'birthday_widget' => "Birthday Widget",
		
		'birthday_widget:widget:title' => "Birthdays",
		'birthday_widget:widget:description' => "Show the upcomming birthdays of site members",
		
		'birthday_widget:widget:settings:who_to_show' => "Show birthdays of:",
		
		'birthday_widget:settings:metadatafield' => "Metadata field which contains the birthdate",
		'birthday_widget:settings:timeformat' => "Time format of the metadata field",
		'birthday_widget:settings:timeformat:unix' => "UNIX (eq. %s)",
		'birthday_widget:settings:timeformat:other' => "Other",
		'birthday_widget:settings:timeformat:description' => "If you chose 'other' as the date format, please supply how this string is formatted based on <a href='%s' title='PHP.net' target='_blank'>this manual</a> on PHP.net",
	);
	
	add_translation("en", $english);
